...
| Gameplay 场景配置 | `GameAppConfig` | [`../GameFlow/GameAppConfig.cs`](../GameFlow/GameAppConfig.cs) |
...
1. 创建 `GameAppConfig.asset`，建议放在 `Assets/Res/Config/App/`；它只配置 Gameplay 场景。
2. 创建并加入 Build Settings 的 Gameplay 场景。
3. 在启动场景中添加 `GameEntry` 和 `UIService`。
4. 给 `GameEntry` 绑定：
...
5. 创建并注册以下 UI Prefab：
   ...
   - `Pause`
   - `Settings`
6. 在 `GameResourceCatalog` 和 `UIService` 中建立 PageId 与 ResourceId 对应关系。页面 ID 由使用它的流程状态自行声明，不放进 `GameAppConfig`。
